...
class DepthNet(nn.Module):
    ...
    def forward(self, x, mats_dict,scale_depth_factor=1000.0):
        mats_dict['intrin_mats'] = torch.tensor(mats_dict['intrin_mats'],device=x.device)  # [12,4,4]
        ida_mats = torch.stack(mats_dict['ida_mats'], dim=0).to(x.device)
        # [6,512,16,44] [num_cam,c,h,w]
        x = self.reduce_conv(x.squeeze())
        # context : [6,80,h,w]
        context = self.context_conv(x)
        # mats_dict['intrin_mats'] : [1,2,6,4,4] [bs,num_sweep,numcam,4,4]
        inv_intrinsics = torch.inverse(mats_dict['intrin_mats'])  # [12,4,4]
        # pixel_size:[6,1]
        pixel_size = torch.norm(torch.stack(
            [inv_intrinsics[..., 0, 0], inv_intrinsics[..., 1, 1]], dim=-1),
                                dim=-1).reshape(-1, 1)
        # aug_scale:[6,1]
        aug_scale = torch.sqrt(ida_mats[...,0, 0]**2 +
                               ida_mats[..., 0, 0]**2).reshape(-1, 1)
        scaled_pixel_size = pixel_size * scale_depth_factor / aug_scale.to(pixel_size.device)
        # x_se [6,512,1,1]
        x_se = self.mlp(scaled_pixel_size.to(x.device).float())[..., None, None]
        # x:[6,512,16,44]
        x = self.se(x, x_se)
...
class Detr3DHeadPECAM(DETRHead):
    """Head of Detr3D.
    Args:
    ...
    def __init__(self,
        ...
        super(Detr3DHeadPECAM, self).__init__(
            *args, transformer=transformer, **kwargs)
        self.code_weights = nn.Parameter(torch.tensor(
            self.code_weights, requires_grad=False), requires_grad=False)

        # self.scale_pred = scale_pred
        self.scale_pred = False
    ...
    def _init_layers(self):
        ...
        self.depth_net = DepthNet(256,256,80)
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        # -------------------cam aware------------------------------
        mats_dict = {}
        mats_dict['intrin_mats'] = img_metas[0]['intrinsics']
        mats_dict['ida_mats'] = img_metas[0]['ida_mats']

        for idx in range(len(mlvl_feats)):
            mlvl_feats[idx] = self.depth_net(mlvl_feats[idx],mats_dict).unsqueeze(0)
            # mlvl_feats[idx] = mlvl_feats[idx] + cam_aware_feature
    ...
    def cam_aware_feature(self,x,mats_dict,scale_depth_factor=1000.0):
