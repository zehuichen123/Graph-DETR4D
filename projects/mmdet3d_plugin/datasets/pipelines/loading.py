        ...
        if self.pad_empty_sweeps and len(results['sweeps']) == 0:
            ...
            for idx in choices:
                ...
                for sensor in self.sensors:
                    sweep_cam = sweep[sensor]       # sweep frame
                    cam_info = results['cam_infos'][sensor] # curr_frame

                    sweep_cam['cam_intrinsic'] = cam_info['cam_intrinsic']

                    e2g_r_s = sweep_cam['ego2global_rotation']
                    e2g_t_s = sweep_cam['ego2global_translation']
                    
                    e2g_t = cam_info['ego2global_translation']
                    e2g_r = cam_info['ego2global_rotation']

                    l2e_t = results['lidar2ego_translation']
                    l2e_r = results['lidar2ego_rotation']
                    ...
                    results['lidar2img'].append(np.array(sweep_cam['lidar2img']))
                    results['intrinsics'].append(np.array(sweep_cam['intrinsics']))
                    results['extrinsics'].append(np.array(sweep_cam['extrinsics']))
...
@PIPELINES.register_module()
class LoadMultiViewImageFromMultiSweepsFilesV2(object):
    """Load multi channel images from a list of separate channel files.
    Expects results['img_filename'] to be a list of filenames.
    Args:
        to_float32 (bool): Whether to convert the img to float32.
